...
abstract: >
    With the \code{R} package mHMMbayes you can fit multilevel hidden Markov models. The multilevel hidden Markov model (HMM) is a generalization of the well-known hidden Markov model, tailored to accommodate (intense) longitudinal data of multiple individuals simultaneously. Using a multilevel framework, we allow for heterogeneity in the model parameters (transition probability matrix and conditional distribution), while estimating one overall HMM. The model has a great potential of application in many fields, such as the social sciences and medicine. The model can be fitted on multivariate data with a categorical distribution, and can include individual level covariates (allowing for e.g., group comparisons on model parameters). Parameters are estimated using Bayesian estimation utilizing the forward-backward recursion within a hybrid Metropolis-within-Gibbs sampler. The package also includes a function to simulate data and a function to obtain the most likely hidden state sequence for each individual using the Viterbi algorithm. 
output: 
...
<br> <br>
```{r include=FALSE}
source('~/Desktop/mHMMbayes/R/sim_mHMM.R', echo=F)
source('~/Desktop/mHMMbayes/R/int_to_prob.R', echo=F)
source('~/Desktop/mHMMbayes/R/mHMM_cont.R', echo=F)
source('~/Desktop/mHMMbayes/R/summary.mHMM.R', echo=F)
source('~/Desktop/mHMMbayes/R/utility_func_mHMM.R', echo=F)
source('~/Desktop/mHMMbayes/R/forward_prob.R', echo=F)
source('~/Desktop/mHMMbayes/R/forward_prob_cpp.R', echo=F)
source('~/Desktop/mHMMbayes/R/RcppExports.R', echo=F)
devtools::install_github("emmekeaarts/mHMMbayes@develop")


```

```{r}
#summary.mHMM(out_2st_c)
# summary(out_2st)
```

## Introduction
Hidden Markov models [HMMs; @Rabiner1989] are a machine learning method that have been used in many different scientific fields for several decades to describe a sequence of observations. For example, translating a fragment of spoken words into text [i.e., speech recognition, see e.g. @Rabiner1989; @woodland2002], or the identification of the regions of DNA that encode genes [i.e., gene tagging, see e.g., @krogh1994; @henderson1997; @burge1998]. The development of this package is, however, motivated from the area of social sciences. Due to technological advancements, it is becoming increasingly easy to collect long sequences of data on behavior. That is, we can monitor behavior as it unfolds in real time. An example is the interaction between a therapist and a patient, where different types of nonverbal communication are registered every second for a period of 15 minutes. When applying HMMs to such behavioral data, they can be used to extract latent behavioral states over time, and model the dynamics of behavior over time. \
...
Given data of multiple subjects, one may fit the HMM to the data of each subject separately, or fit one and the same HMM model to the data of all subject, under the strong (generally untenable) assumption that the subjects do not differ with respect to the parameters of the HMM. Fitting a different model to each behavioral sequence is not parsimonious, is computationally intensive, and results in a large number of parameters estimates. Neither approach lends itself well for a formal comparison (e.g., comparing the parameters over experimental conditions). To facilitate the analysis of multiple subjects, the HMM is extended by putting it in a multilevel framework. \
...
As mentioned earlier, it can also be of interest to include covariates in the model. This way, we can investigate the impact of these covariates on the model parameters. For example, using the `nonverbal` data, we can assess whether the dynamics between a patient and a therapist differ for patients depending on their therapeutic outcome or their (standardized) changes in CDI and SCA. For now, we will fit a model including only the covariate `diagnosis`. \

We specify the same general model properties and starting values as when we fitted the 2-state HMM with no covariates earlier:
...
Now, we proceed to define the covariate in such a way that it may be included within the function `mHMM`, and we run the function. 
```{r}
# Run a model including a covariate. Here, the covariate (diagnosis)
#predicts the emission distribution and transition probability distribution for each of the 4 dependent
# variables:

n_subj <- 10
xx <- rep(list(matrix(1, ncol = 1, nrow = n_subj)), (n_dep + 1))
for(i in 1:(n_dep + 1)){
  xx[[i]] <- cbind(xx[[i]], nonverbal_cov$diagnosis)
}
...
```
We can achieve the same graphical output as before: 
```{r, fig.width = 7.2, fig.height = 4}
Voc_col <- c(brewer.pal(3,"PuBuGn")[c(1,3,2)])
Voc_lab <- c("Not Speaking", "Speaking", "Back channeling")

plot(out_2st_c, component = "emiss", dep = 1, col = Voc_col, 
     dep_lab = c("Patient vocalizing"), cat_lab = Voc_lab)
```

What we are particularly interested in here, though, is visualizing the effect of the covariate on the transition probabilities $\gamma_{k,ij}$ and the state-dependent probabilities $\theta_{ki}$. First, we plot the transition probabilities $\gamma_{k,ij}$ as a function of the covariate `diagnosis`. 

```{r}
burn_in <- 5
mean_beta <- colMeans(out_2st_c$gamma_cov_bar[-(1:burn_in), ])
mean_int <- colMeans(out_2st_c$gamma_int_bar[-(1:burn_in), ])
cov <- as.vector(c(0, 1))

#this provides the mean of alpha_(Gamma)ki, so we should then need to do a random draw from a normal dist
gamma_alpha_ki <- matrix(NA, nrow = 2, ncol = 2)
for (i in 1:2){
  gamma_alpha_ki[i,] <-  mean_int[i] + mean_beta[i]*cov
}

#this converts the above values into probabilities via the logit transform
gamma_cov <- matrix(NA, nrow = 2, ncol = 2)
for (i in 1:nrow(gamma_cov)){
  for (j in 1:ncol(gamma_cov)){
    gamma_cov[i,j] <- exp(gamma_alpha_ki[i,j])/(1+exp(gamma_alpha_ki[i,j]))
  }
}
gamma_cov[2,] <- 1 - gamma_cov[2,]
# rownames(gamma_cov) <- c("gamma12", "gamma21")
# colnames(gamma_cov) <- c("diagnosis0", "diagnosis1")

gamma_cov_s1 <- matrix(NA, nrow = 2, ncol = 2)
gamma_cov_s1[1, ] <- 1 - gamma_cov[1, ]
gamma_cov_s1[2, ] <- gamma_cov[1, ]

gamma_cov_s2 <- matrix(NA, nrow = 2, ncol = 2)
gamma_cov_s2[1, ] <- 1 - gamma_cov[2, ]
gamma_cov_s2[2, ] <- gamma_cov[2, ]
```


```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov, y = gamma_cov_s1[1, ], type = "b",ylim = c(0, 1), xlim = c(0, 1), xaxt = 'n', ylab = "Transition probability", xlab = "Diagnosis")
lines(x = cov, y = gamma_cov_s1[2, ], type = "b")
axis(side = 1, at = c(0, 1))
```

```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov, y = gamma_cov_s2[1, ], type = "b",ylim = c(0, 1), xlim = c(0, 1), xaxt = 'n', ylab = "Transition probability", xlab = "Diagnosis")
lines(x = cov, y = gamma_cov_s2[2, ], type = "b")
axis(side = 1, at = c(0, 1))
```

Next, we plot the state-dependent probabilities $\theta_{ki}$ as a function of `diagnosis`. 

```{r}
mean_cov_emiss <- vector("list", n_dep)
mean_int_emiss <- vector("list", n_dep)
cov <- as.vector(c(0, 1))
emiss_alpha_ki <- vector("list", n_dep)
emiss_cov <- vector("list", n_dep)
emiss_cov_prob <- vector("list", n_dep)

for (i in 1:n_dep){
  mean_cov_emiss[[i]] <- colMeans(out_2st_c$emiss_cov_bar[[i]][-(1:burn_in), ])
  mean_int_emiss[[i]] <- colMeans(out_2st_c$emiss_int_bar[[i]][-(1:burn_in), ])
  
  emiss_alpha_ki[[i]] <- matrix(NA, ncol = length(mean_cov_emiss[[i]]), nrow = 2)
  emiss_cov[[i]] <- matrix(NA, ncol = length(mean_cov_emiss[[i]])/2, nrow = 4)
  emiss_cov_prob[[i]] <- matrix(NA, ncol = q_emiss[i], nrow = 4)
  
  #this provides the mean of alpha_(Gamma)ki, so we should then need to do a random draw from a normal dist
  for (j in 1:length(mean_cov_emiss[[i]])){
    emiss_alpha_ki[[i]][,j] <-  mean_int_emiss[[i]][j] + mean_cov_emiss[[i]][j]*cov
  }
  
  for (j in 1:2){
    emiss_cov[[i]][j, ] <- emiss_alpha_ki[[i]][j , 1:(q_emiss[i]-1)]
  }
  for (j in 3:4){
        emiss_cov[[i]][j, ] <- emiss_alpha_ki[[i]][j-2, -(1:(q_emiss[i]-1))]
  }

  emiss_cov_prob[[i]] <- apply(emiss_cov[[i]], 1, int_to_prob)
  rownames(emiss_cov_prob[[i]]) <- paste("Category", 1:q_emiss[i])
  colnames(emiss_cov_prob[[i]]) <- c("S1, Diagnosis = 0", "S1, Diagnosis = 1", "S2, Diagnosis = 0", "S2, Diagnosis = 1")
  names(emiss_cov_prob) <- out_2st_c$input$dep_labels
}
...
for (i in 1:n_dep){
  for (j in 1:4){
    plot(x = as.factor(rownames(emiss_cov_prob[[i]])), y = emiss_cov_prob[[i]][, j], type = "p",ylim = c(0, 1), ylab = "Probability", xlab = "Category", main = paste(names(emiss_cov_prob)[i]), colnames(emiss_cov_prob[[i]])[j])
  }
}
```

The same effect can be achieved if we were to have used a different covariate, standardized CDI change, say.
```{r}
# Run a model including a covariate. Here, the covariate (standardized CDI
# change) predicts the emission distribution and transition probability dist. for each of the 4 dependent
# variables:
...
out_2st_c1 <- mHMM(s_data = nonverbal, xx = xx,
                  gen = list(m = m, n_dep = n_dep, q_emiss = q_emiss),
                  start_val = c(list(start_TM), start_EM),
                  mcmc = list(J = 11, burn_in = 5))
...
```{r}
burn_in <- 5
mean_beta1 <- colMeans(out_2st_c1$gamma_cov_bar[-(1:burn_in), ])
mean_int1 <- colMeans(out_2st_c1$gamma_int_bar[-(1:burn_in), ])
cov1 <- nonverbal_cov$std_CDI_change

#this provides the mean of alpha_(Gamma)ki, so we should then need to do a random draw from a normal dist
gamma_alpha_ki1 <- matrix(NA, nrow = 2, ncol = length(cov1))
for (i in 1:2){
  gamma_alpha_ki1[i,] <-  mean_int1[i] + mean_beta1[i]*cov1
}

#this converts the above values into probabilities via the logit transform
gamma_cov1 <- matrix(NA, nrow = 2, ncol = length(cov1))
for (i in 1:nrow(gamma_cov1)){
  for (j in 1:ncol(gamma_cov1)){
    gamma_cov1[i,j] <- exp(gamma_alpha_ki1[i,j])/(1+exp(gamma_alpha_ki1[i,j]))
  }
}
gamma_cov1[2,] <- 1 - gamma_cov1[2,]
# rownames(gamma_cov) <- c("gamma12", "gamma21")
# colnames(gamma_cov) <- c("diagnosis0", "diagnosis1")

gamma_cov1_s1 <- matrix(NA, nrow = 2, ncol = length(cov1))
gamma_cov1_s1[1, ] <- 1 - gamma_cov1[1, ]
gamma_cov1_s1[2, ] <- gamma_cov1[1, ]

gamma_cov1_s2 <- matrix(NA, nrow = 2, ncol = length(cov1))
gamma_cov1_s2[1, ] <- 1 - gamma_cov1[2, ]
gamma_cov1_s2[2, ] <- gamma_cov1[2, ]
```

```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov1, y = gamma_cov1_s1[1, ], type = "b",ylim = c(0, 1), xlim = c(-2, 2), xaxt = 'n', ylab = "Transition probability", xlab = "Standardized CDI change")
lines(x = cov1, y = gamma_cov1_s1[2, ], type = "b")
axis(side = 1, at = c(-2, -1, 0, 1, 2))
```

```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov1, y = gamma_cov1_s2[1, ], type = "b",ylim = c(0, 1), xlim = c(-2, 2), xaxt = 'n', ylab = "Transition probability", xlab = "Standardized CDI change")
lines(x = cov1, y = gamma_cov1_s2[2, ], type = "b")
axis(side = 1, at = c(-2, -1, 0, 1, 2))
```

```{r}
mean_cov_emiss1 <- vector("list", n_dep)
mean_int_emiss1 <- vector("list", n_dep)
cov1 <- nonverbal_cov$std_CDI_change
emiss_alpha_ki1 <- vector("list", n_dep)
emiss_cov1 <- vector("list", n_dep)
emiss_cov_prob1 <- vector("list", n_dep)

for (i in 1:n_dep){
  mean_cov_emiss1[[i]] <- colMeans(out_2st_c1$emiss_cov_bar[[i]][-(1:burn_in), ])
  mean_int_emiss1[[i]] <- colMeans(out_2st_c1$emiss_int_bar[[i]][-(1:burn_in), ])
  
  emiss_alpha_ki1[[i]] <- matrix(NA, ncol = length(mean_cov_emiss1[[i]]), nrow = 10)
  emiss_cov1[[i]] <- matrix(NA, ncol = length(mean_cov_emiss1[[i]])/2, nrow = 20)
  emiss_cov_prob1[[i]] <- matrix(NA, ncol = q_emiss[i], nrow = 20)
  
  #this provides the mean of alpha_(Gamma)ki, so we should then need to do a random draw from a normal dist
  for (j in 1:length(mean_cov_emiss1[[i]])){
    emiss_alpha_ki1[[i]][,j] <-  mean_int_emiss1[[i]][j] + mean_cov_emiss1[[i]][j]*cov1
  }
  
  for (j in 1:10){
    emiss_cov1[[i]][j, ] <- emiss_alpha_ki1[[i]][j , 1:(q_emiss[i]-1)]
  }
  for (j in 11:20){
        emiss_cov1[[i]][j, ] <- emiss_alpha_ki1[[i]][(j-10), -(1:(q_emiss[i]-1))]
  }

  emiss_cov_prob1[[i]] <- apply(emiss_cov1[[i]], 1, int_to_prob)
  rownames(emiss_cov_prob1[[i]]) <- paste("Category", 1:q_emiss[i])
  #colnames(emiss_cov_prob1[[i]]) <- c("S1, Diagnosis = 0", "S1, Diagnosis = 1", "S2, Diagnosis = 0", "S2, Diagnosis = 1")
  #names(emiss_cov_prob1) <- out_2st_c$input$dep_labels
}
...
```{r, fig.width = 7.2, fig.height = 4}
for (i in 1:n_dep){
  plot(x = c(1:nrow(emiss_cov_prob1[[i]])), y = emiss_cov_prob1[[i]][, 1], type = "p", pch = 1, ylim = c(0, 1), ylab = "Probability", xaxt = "none", main = paste(names(emiss_cov_prob1)[i]), colnames(emiss_cov_prob1[[i]])[1])
  axis(2, rownames(emiss_cov_prob1[[i]])[1])
  for (j in 2:10){
    lines(x = c(1:nrow(emiss_cov_prob1[[i]])), y = emiss_cov_prob1[[i]][, j], type = "p", pch = 1, ylim = c(0, 1), ylab = "Probability", xaxt = "none", xlab = rownames(emiss_cov_prob1[[i]]), main = paste(names(emiss_cov_prob1)[i]), colnames(emiss_cov_prob1[[i]])[j])
      axis(2, rownames(emiss_cov_prob1[[i]])[j])
  }
}
```

### Simulating data with the package `mHMMbayes`

The package includes a function for simulating data. This can provide examples of different scenarios to test the multilevel HMM model on. 

One example in which we require this function is when we want to run our model with a continuous emission distribution $\boldsymbol{\theta}_{k,i}$ (i.e. subject-specific emission distributions denoting subject-specific probabilities $\boldsymbol{\theta}_{k,i}$ of continuous outcomes within hidden state $i$).\

We will illustrate how this type of data can be simulated with the `mHMMbayes` package, and in the next section we will fit the mHMM model to this simulated data. \

In the first four lines of code we define the length of the observed sequence to be simulated for each subject, the number of subjects, the number of states and the number of dependent variables, respectively. Next, values for the average transition probability matrix (`gamma`) and emission distributions (`emiss_distr`) are specified. Other input in the sim_mHMM() function includes `var_gamma` and `var_emiss`, which represent the variance of the transition probability matrix and emission distributions, respectively. The input data_distr = 'continuous' is required when simulating continuous data. 
...
### Using a continuous emission distribution
In the previous example, we fitted our multilevel Hidden Markov Model on example data where the subject-specific emission distributions denoted subject-specific probabilities $\boldsymbol{\theta}_{k,i}$ of categorical outcomes within hidden state $i$. However, as we will now show, this is also possible for continuous emission distributions. To demonstrate this, we will apply our model to the data we just simulated. 
