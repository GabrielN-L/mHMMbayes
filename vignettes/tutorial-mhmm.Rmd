...
```{r}
mean_beta <- colMeans(out_2st_c$gamma_cov_bar)
mean_int <- colMeans(out_2st_c$gamma_int_bar)
cov <- as.vector(c(0, 1))
...
gamma_cov1 <- matrix(NA, nrow = 2, ncol = 2)
gamma_cov1[1, ] <- 1 - gamma_cov[1, ]
gamma_cov1[2, ] <- gamma_cov[1, ]

gamma_cov2 <- matrix(NA, nrow = 2, ncol = 2)
gamma_cov2[1, ] <- 1 - gamma_cov[2, ]
gamma_cov2[2, ] <- gamma_cov[2, ]
```


```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov, y = gamma_cov1[1, ], type = "b",ylim = c(0, 1), xlim = c(0, 1), xaxt = 'n', ylab = "Transition probability", xlab = "Diagnosis")
lines(x = cov, y = gamma_cov1[2, ], type = "b")
axis(side = 1, at = c(0, 1))
```

```{r, fig.width = 2.5, fig.height = 4}
plot(x = cov, y = gamma_cov2[1, ], type = "b",ylim = c(0, 1), xlim = c(0, 1), xaxt = 'n', ylab = "Transition probability", xlab = "Diagnosis")
lines(x = cov, y = gamma_cov2[2, ], type = "b")
axis(side = 1, at = c(0, 1))
```
...
source('~/Desktop/mHMMbayes/R/sim_mHMM.R', echo=TRUE)
source('~/Desktop/mHMMbayes/R/int_to_prob.R', echo=TRUE)
...
```{r}
source('~/Desktop/mHMMbayes/R/mHMM_cont.R', echo=TRUE)
data_cont <- sim_mHMM(n_t = n_t, n = n, m = m, n_dep = n_dep, data_distr = 'continuous',
